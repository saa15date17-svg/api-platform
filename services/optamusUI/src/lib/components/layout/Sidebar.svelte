<script lang="ts">
	...
	import Usage from '../icons/Usage.svelte';
	import { slide } from 'svelte/transition';
	import HotkeyHint from '../common/HotkeyHint.svelte';

	const BREAKPOINT = 768;
	const DEFAULT_PINNED_ITEMS = ['home', 'models', 'api-keys', 'logs', 'usage', 'notes'];
	...
	const getMenuItemMeta = (id) => {
		const items = {
			...
			home: { label: 'Home', href: '/home', iconType: 'home' },
			models: { label: 'Models', href: '/models', iconType: 'models' },
			'api-keys': { label: 'API Keys', href: '/api-keys', iconType: 'api-keys' },
			logs: { label: 'Logs', href: '/logs', iconType: 'logs' },
			usage: { label: 'Usage', href: '/usage', iconType: 'usage' },
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									...
									>
										<div class="self-center">
											{#if itemId === 'notes'}
												<Note className="size-4.5" strokeWidth="1.5" />
											{:else if itemId === 'home'}
												<svg
													xmlns="http://www.w3.org/2000/svg"
													fill="none"
											...
											{:else if itemId === 'api-keys'}
												<Key className="size-4.5" strokeWidth="1.5" />
											{:else if itemId === 'logs'}
												<Logs className="size-4.5" strokeWidth="1.5" />
											{:else if itemId === 'usage'}
												<Usage className="size-4.5" strokeWidth="1.5" />
											{:else if itemId === 'automations'}
												<svg
													xmlns="http://www.w3.org/2000/svg"
													fill="none"
											...
											{:else if itemId === 'playground'}
												<Code className="size-4.5" strokeWidth="1.5" />
											{:else if itemId === 'models'}
												<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-4.5">
													<path stroke-linecap="round" stroke-linejoin="round" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-9v9" />
												</svg>
