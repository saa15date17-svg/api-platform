...
async def get_system_oauth_token(request, user):
    ...
    SSO OAuth is now handled by Zitadel. This function returns None
    as the legacy SSO OAuthManager has been removed.
    """
    return None
        ...
        try:
            ...
            if choices and (content or response_output):
                if content or response_output:
                    ...
                    if usage:
                        try:
                            model_id = response_data.get('model', form_data.get('model', ''))
