        ...
        async def async_embedding_function(query, prefix=None, user=None):
            ...
            if embedding_function is None:
                raise ValueError(
                    ...
                    'RAG_EMBEDDING_ENGINE (ollama, openai, azure_openai, gemini).'
                )
...
async def agenerate_gemini_batch_embeddings(
    ...
    log.debug(f'agenerate_gemini_batch_embeddings:model {model} batch size: {len(texts)}')
    try:
    ...
    **kwargs,
    ...
    if engine == 'ollama':
        ...
        embeddings = await agenerate_gemini_batch_embeddings(
            model, text if isinstance(text, list) else [text], key, prefix, user
        )
        if embeddings is None:
            return None
        return embeddings[0] if isinstance(text, str) else embeddings
