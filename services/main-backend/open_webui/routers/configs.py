...
UI_CONFIG_KEYS = {
    'platform_name': 'ui.platform_name',
    'support_email': 'ui.support_email',
}


@router.get('/ui', response_model=UIConfigForm)
async def get_ui_config(user=Depends(get_admin_user)):
    values = await Config.get_many(*UI_CONFIG_KEYS.values())
    return UIConfigForm(
        platform_name=values.get('ui.platform_name'),
        support_email=values.get('ui.support_email'),
    )


@router.post('/ui', response_model=UIConfigForm)
async def set_ui_config(request: Request, form_data: UIConfigForm, user=Depends(get_admin_user)):
    updates = {UI_CONFIG_KEYS[field]: value for field, value in form_data.model_dump().items() if value is not None}
    if updates:
            ...
            if form_data.auth_type in ('oauth_2.1', 'oauth_2.1_static'):
                ...
                try:
                    ...
                    if form_data.auth_type == 'bearer':
                        ...
                        # SSO OAuth is now handled by Zitadel — no token to forward
                        pass
                    if token:
...
async def update_admin_config(
    request: Request,
    form_data: AdminConfigForm,
    user=Depends(get_admin_user),
):
    data = form_data.model_dump()
    await Config.upsert(data['data'])
    await publish_event(
        request,
        EVENTS.ADMIN_CONFIG_UPDATED,
        actor=user,
        subject_id='admin.config',
        subject_type='config',
