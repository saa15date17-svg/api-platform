<script>
		...
		if (type === 'calendar:alert' && data) {
			...
			if ($isLastActiveTab) {
				if ($settings?.notificationEnabled ?? false) {
					new Notification(`${data.title} • Optamus AI`, {
						body: timeStr,
		...
		if ((event.chat_id !== $chatId && !$temporaryChatEnabled) || isInBackground) {
			if (type === 'chat:completion') {
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${displayTitle} • Optamus AI`, {
								body: contentPreview,
								icon: `${WEBUI_BASE_URL}/static/favicon.png`
							});
						}
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isInBackground) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • Optamus AI`, {
							body: data?.content,
	...
	const windowMessageEventHandler = async (event) => {
		if (
		!['https://optamus.cloud', 'https://www.optamus.cloud', 'http://localhost:9999'].includes(
			event.origin
		)
		) {
			return;
